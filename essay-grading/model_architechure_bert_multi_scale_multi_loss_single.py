...
class DocumentBertScoringModel():
    def __init__(self, args=None):
        ...
        self.config = config
        self.prompt = int(args.prompt[1])  # prompt: p8 --> prompt[1]: 8
        chunk_sizes_str = self.args['chunk_sizes']  # 90_30_130_10
        self.chunk_sizes = []
    ...
    def predict_for_regress(self, data):
        ...
        with torch.no_grad():
            print(f"document_representations_word_document.shape[0]: {document_representations_word_document.shape[0]}")
            ...
            for i in range(0, document_representations_word_document.shape[0], self.args['batch_size']):
                ...
                # print(f"document_representations_word_document.shape: {document_representations_word_document.shape}")
                # print(f"document_representations_word_document[0, 0]: {document_representations_word_document[0, 0]}")
                # print(f"document_representations_word_document[0, 1]: {document_representations_word_document[0, 1]}")
                # print(f"batch_predictions_word_document: {batch_predictions_word_document}")

                batch_predictions_word_chunk_sentence_doc = batch_predictions_word_document
                for chunk_index in range(len(self.chunk_sizes)):
                    print(f"self.chunk_sizes: {self.chunk_sizes}")
