...
class DocumentBertSentenceChunkAttentionLSTM(BertPreTrainedModel):
    ...
    def forward(self, document_batch: torch.Tensor, device='cpu', bert_batch_size=0):
        ...
        attention_hidden = torch.sum(attention_hidden, dim=1)  # 加权求和 (batch_size, num_hiddens)  # attention_hidden = hidden dim d = 768
        print(f"attention_hidden.shape: {attention_hidden.shape}")  # (N, 768)
...
class DocumentBertCombineWordDocumentLinear(BertPreTrainedModel):
    ...
    def forward(self, document_batch: torch.Tensor, device='cpu'):
        ...
        print(f"all_bert_output_info[0].shape: {all_bert_output_info[0].shape}")  # torch.Size([1, 512, 768])
        print(f"all_bert_output_info[1].shape: {all_bert_output_info[1].shape}")  # torch.Size([1, 768])
        print(f"bert_output.view(bert_output.shape[0], -1).shape: {bert_output.view(bert_output.shape[0], -1).shape}")
